...
const AuthProviderIcon = ({ jwt, iconSize = 64, showStatus, wallet, }) => {
    ...
    if (!walletForIcon) {
        if (isLoadingEmbeddedWallet) {
            return (jsxRuntime.jsx(WalletIconWithNetwork.WalletIconWithNetwork, { iconSize: iconSize, Icon: embeddedWalletIcon.ReactComponent, chainName: 'eip155', showNetwork: false }));
        }
        return null;
    }
    ...
    if (isEmbeddedWallet && MagicEmailOrSocialIcon) {
        ...
        return (jsxRuntime.jsx(WalletIconWithNetwork.WalletIconWithNetwork, { iconSize: iconSize, Icon: MagicEmailOrSocialIcon, chainName: chainEnum, showNetwork: showStatus }));
    }
