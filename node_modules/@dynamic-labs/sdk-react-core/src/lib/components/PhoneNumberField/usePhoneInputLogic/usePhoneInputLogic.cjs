...
const customCountries = getCustomCountries.getCustomCountries();
/** The default country we want to use */
const getDefaultCountry = () => {
    var _a;
    const defaultCountryIso2 = (_a = localStorage.LocalStorage.getFromLS(localStorage$1.PHONE_INPUT_DEFAULT_COUNTRY)) !== null && _a !== void 0 ? _a : 'us';
    return customCountries
        .map(reactInternationalPhone.parseCountry)
...
const usePhoneInputLogic = ({ countryInputRef, setCountryDropdownOpen, setInvalidNumberMessage, onUpdate, initialPhone, }) => {
    ...
    const [countryInput, setCountryInput] = React.useState((_d = (_c = getDefaultCountry()) === null || _c === void 0 ? void 0 : _c.dialCode) !== null && _d !== void 0 ? _d : '1');
    ...
    const lastValidCountry = React.useRef(getDefaultCountry());
    const handleCountryInput = ({ target, }) => {
        ...
        localStorage.LocalStorage.setToLS(localStorage$1.PHONE_INPUT_DEFAULT_COUNTRY, guessedCountry.iso2);
    ...
    const resetToLastValidCountry = () => {
        ...
        localStorage.LocalStorage.setToLS(localStorage$1.PHONE_INPUT_DEFAULT_COUNTRY, lastValidCountry.current.iso2);
    ...
    const handleArrowInput = React.useCallback((event) => {
        ...
        localStorage.LocalStorage.setToLS(localStorage$1.PHONE_INPUT_DEFAULT_COUNTRY, newCountry.iso2);
        // Changing country resets the phone number, so we must also reset any errors
        setInvalidNumberMessage(undefined);
    }, [
        currentCountry.iso2,
