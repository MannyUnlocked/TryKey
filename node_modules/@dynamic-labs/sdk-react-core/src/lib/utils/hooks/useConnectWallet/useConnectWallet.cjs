...
const useConnectWallet = ({ authMode, clearPrimaryWalletId, enableVisitTrackingOnConnectOnly, environmentId, primaryWalletId, setPrimaryWalletId, walletConnectorOptions, onDisconnect, onBeforeConnectSuccessConfirmation, removeIsFullyConnected, setShowAuthFlow, isBridgeFlow, setIsVerificationInProgress, user, }) => {
    ...
    /**
     * When a new wallet is connected with the onConnect callback enabled,
     * this new wallet's id will hang in this array until we attest that the corresponding wallet object
     * has been successfully built — at which point we raise the callback and pop the id from this array.
     * Should be removed with QNTM-1082 refactor
     */
    const onConnectWaitList = React.useRef(new Set());
    ...
    const disconnectWallet = React.useCallback((walletId) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        ...
        if (updatedConnectedWalletsInfo.length === 0) {
    ...
    const updateConnectedWalletsList = React.useCallback(() => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        ...
        const updatedConnectedWallets = (yield Promise.all(connectedWalletsInfo.map((storedConnectedWalletInfo) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
            ...
            const [walletNetwork, additionalAddresses] = yield Promise.all([
                walletConnector.getNetwork(),
                walletConnector.getAdditionalAddresses(walletAddress),
            ]);
        ...
        // Raise any onConnect callbacks in wait list
        for (const walletId of [...onConnectWaitList.current]) {
            const wallet = updatedConnectedWallets.find((wallet) => getWalletIdentifier.getWalletIdentifier(wallet) === walletId);
            if (!wallet)
                continue;
            onConnectWaitList.current.delete(walletId);
            dynamicEvents.dynamicEvents.emit('walletConnected', wallet);
    ...
    const updateConnectedWalletById = React.useCallback((walletId, walletObject) => {
        ...
        if (connectedWalletIndex > -1) {
            if (walletObject.address && walletObject.network) {
                clonedConnectedWalletsList[connectedWalletIndex] = Object.assign(Object.assign({}, clonedConnectedWalletsList[connectedWalletIndex]), { address: walletObject.address, network: walletObject.network });
            }
            else if (walletObject.address) {
                clonedConnectedWalletsList[connectedWalletIndex] = Object.assign(Object.assign({}, clonedConnectedWalletsList[connectedWalletIndex]), { address: walletObject.address });
            }
            else {
                clonedConnectedWalletsList[connectedWalletIndex] = Object.assign(Object.assign({}, clonedConnectedWalletsList[connectedWalletIndex]), { network: walletObject.network });
            }
    ...
    const refreshConnectedWallet = (walletId, walletConnector) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        ...
        if (currentWalletConnectorAddress && onBeforeConnectSuccessConfirmation) {
    ...
    const connectWallet = (walletConnector, getAddressOpts, options) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        const { applyOnBeforeConnectSuccessConfirmation = true, raiseOnConnect = false, } = options !== null && options !== void 0 ? options : {};
        ...
        if (onBeforeConnectSuccessConfirmation &&
            applyOnBeforeConnectSuccessConfirmation) {
            const shouldProceedWithConnection = yield onBeforeConnectSuccessConfirmation({
                address: walletAddress,
                chain: walletConnector.connectedChain,
                connector: walletConnector,
        ...
        // We are already using the new syntax for wallet ids for this.
        // Eventually the connect-wallet-X syntax will be dropped
        if (raiseOnConnect && isConnectOnly.isConnectOnly(user, authMode))
            onConnectWaitList.current.add(getWalletIdentifier.getWalletIdentifier({
                address: walletAddress,
                connector: walletConnector,
            }));
        if (!primaryWalletId) {
