...
const useConnectWallet = ({ authMode, clearPrimaryWalletId, enableVisitTrackingOnConnectOnly, environmentId, primaryWalletId, setPrimaryWalletId, walletConnectorOptions, onDisconnect, onBeforeConnectSuccessConfirmation, removeIsFullyConnected, setShowAuthFlow, isBridgeFlow, setIsVerificationInProgress, user, }) => {
     ...
     */
    const onConnectWaitList = useRef(new Set());
    ...
    const updateConnectedWalletsList = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        for (const walletId of [...onConnectWaitList.current]) {
            const wallet = updatedConnectedWallets.find((wallet) => getWalletIdentifier(wallet) === walletId);
            if (!wallet)
                continue;
            onConnectWaitList.current.delete(walletId);
            dynamicEvents.emit('walletConnected', wallet);
    ...
    const connectWallet = (walletConnector, getAddressOpts, options) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (raiseOnConnect && isConnectOnly(user, authMode))
            onConnectWaitList.current.add(getWalletIdentifier({
                address: walletAddress,
                connector: walletConnector,
            }));
        if (!primaryWalletId) {
