...
var farcaster = require('./farcaster.cjs');
require('yup');
...
const useSocialAuth = ({ sessionTimeout, authMode, onSettled, onError, }) => {
    var _a;
    const { environmentId, onboardingOnlyJwt, setOnboardingOnlyJwt, setCallback, setShowAuthFlow, setIsVerificationInProgress, projectSettings, consumeNonce, } = useInternalDynamicContext.useInternalDynamicContext();
    ...
    const connectSocialAccount = ({ provider, validator, captchaToken, }) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        ...
        if (provider === sdkApi.ProviderEnum.Farcaster) {
        ...
        const handleAuthWindowMessage = (event) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
            ...
            if (authMode === 'signin') {
                const apiCall = () => api.signInOAuth(environmentId, provider, {
                    captchaToken,
                    code,
                ...
                const apiCall = () => api.verifyOAuth(environmentId, provider, {
                    code,
                    codeVerifier: verifier,
                    state,
                }, String(userJwt));
    ...
    const connectWithFarcaster = () => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        ...
        if (!(data === null || data === void 0 ? void 0 : data.url) || !(data === null || data === void 0 ? void 0 : data.channelToken)) {
            handleError(types.SocialOAuthErrorCode.GENERAL_ERROR, "Farcaster didn't return a valid url.");
            return;
        }
        ...
        if (utils.isMobile()) {
            ...
            setView('farcaster-connect-view', { url: data.url });
        }
        ...
        if (status === 'timeout' ||
            ...
            handleError(types.SocialOAuthErrorCode.GENERAL_ERROR, "Farcaster didn't return valid data.");
            return;
        }
        const { custody: address, message, signature, nonce } = status;
        if (authMode === 'signin') {
            const apiCall = () => api.farcasterSignIn(environmentId, {
                address,
                domain,
                message,
                nonce,
            ...
            yield signInAccount(sdkApi.ProviderEnum.Farcaster, apiCall);
        }
        else {
            handleError(types.SocialOAuthErrorCode.GENERAL_ERROR, 'Linking with Farcaster is not supported at the moment.');
        }
        setIsProcessing(false);
    });
    return {
