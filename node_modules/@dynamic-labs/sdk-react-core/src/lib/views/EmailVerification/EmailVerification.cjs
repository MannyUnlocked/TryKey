...
const EmailVerification = () => {
    ...
    const { displayedDestination: email, verificationUUID, setVerificationUUID, } = VerificationContext.useVerification();
    ...
    useOnUnmount.useOnUnmount(() => {
        if (!callCallbackOnUnmount.current)
            return;
        dynamicEvents.dynamicEvents.emit('emailVerificationResult', isValidatedRef.current, email !== null && email !== void 0 ? email : '');
    });
    ...
    const handleComplete = React.useCallback((verificationToken) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            const { updateUserProfileResponse } = yield verifyOtp(verificationToken, 'email', verificationUUID);
